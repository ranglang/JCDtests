...
//    @Parameters( method = "nounsData")
    ...
    public void otherNouns(){
        //检查账号类型为保证金账户
        perform(AAsset.ID_TEXT_net_worth , click());
        ViewInteraction d = check(ID_TEXT_account_type ,matches(withText(Details.check_account_type)));


        Matcher<View> a = getAllMatcher(Details.ID_TEXT_individual);
        Matcher<View> b = hasSibling(getAllMatcher(ID_TEXT_account_type));
        sleep(2000);
      //  ViewInteraction c = onView(a).perform(click());

        check(d , matches(b));
//        check(d ,matches(withText(Details.check_account_type)));

//        check(ID_TEXT_account_type ,matches(hasSibling(getAllMatcher(Details.ID_TEXT_individual))));
